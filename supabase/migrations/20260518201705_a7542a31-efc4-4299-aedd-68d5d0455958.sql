CREATE OR REPLACE FUNCTION public.record_stock_movement(_product_id uuid, _movement_type text, _quantity numeric, _unit_cost numeric DEFAULT NULL::numeric, _reason text DEFAULT NULL::text, _reference text DEFAULT NULL::text, _target_quantity numeric DEFAULT NULL::numeric, _supplier_id uuid DEFAULT NULL::uuid, _invoice_number text DEFAULT NULL::text, _lot_number text DEFAULT NULL::text, _expiry_date date DEFAULT NULL::date)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Past-date check only applies to incoming stock (new entries/retours)
  IF _expiry_date IS NOT NULL
     AND _movement_type IN ('entree','retour')
     AND _expiry_date < CURRENT_DATE - INTERVAL '1 day' THEN
